...
当前发布候选为 npm `web-source-inspector@0.1.0-beta.4`，对应手动安装的 VSIX `web-source-inspector-vscode@0.1.1`。正式接入时，目标项目必须本地安装 `web-source-inspector` 作为 npm 开发依赖；VS Code/Cursor 扩展是独立安装的 VSIX，负责调用工作区内 CLI 和打开源码。
...
```text
D:\otherProject\web-source-inspector\web-source-inspector-0.1.0-beta.4.tgz
