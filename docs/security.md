...
精确定位的正式接入前提是目标项目本地安装 `web-source-inspector` 开发依赖并接入其构建配置。VS Code/Cursor 扩展只调用工作区内的 CLI，预览 diff 后等待用户确认；它不自动安装 npm 包、不使用全局 CLI，也不提供仅安装插件、零项目接入路径。初始化不修改业务源码，Inspector 也不进入生产构建。
...
- raw Webpack watch 只接受精确 HTTP Origin；HTTPS Origin 返回 `RAW_WATCH_HTTPS_UNSUPPORTED`，不能由代理或端口转发放宽。

页面内脚本能够读取当前 DOM 的 sourceId，因此 sourceId 不是认证凭据。安全性来自它只能索引当前 Manifest、Bridge 不暴露给浏览器以及 IDE 端二次路径校验。

## 远程浏览器

默认 `browserAccess: 'same-machine'`，Browser Router 只接受回环地址和启动时冻结的本机网卡地址；非回环 socket 必须与 Vite 页面 HTTP(S) Origin 的字面量 IP、协议和实际 listener 端口精确一致。插件不改写 Vite `server.host`，项目必须自行决定是否暴露对应接口。Vite 可显式使用 `browserAccess: 'loopback'` 收紧为只接受回环客户端。
...
- 没有打开 workspace 时不连接。
- WSL、Docker、Remote SSH、Dev Container、Codespaces、手机和其它机器上的浏览器等首版明确拒绝。
...
Vite Adapter 只在真实 `serve` 生命周期创建 session，`build`、`preview` 和 `enabled: false` 保持 no-op；Webpack Adapter 在非 `development` 模式保持 no-op，只在 development Dev Server/受支持 watch transport 中启用。发布消费应用前仍要构建并检查实际产物。以下命令中的 `<app>`、`<dist>` 替换为消费项目脚本和输出目录：
