...
```text
D:\otherProject\web-source-inspector\web-source-inspector-0.1.0.tgz
```
...
当前版本尚未发布到 npm、VS Code Marketplace、Open VSX 或 Cursor Marketplace，因此请先使用以上本地文件安装。
...
```powershell
cd D:\your-project
npm install -D "D:\otherProject\web-source-inspector\web-source-inspector-0.1.0.tgz"
```

使用 pnpm 或 yarn 时也可以安装同一个 `.tgz` 文件：

```powershell
pnpm add -D "D:\otherProject\web-source-inspector\web-source-inspector-0.1.0.tgz"
```
...
- WSL、Remote SSH、Dev Container、Codespaces 和远程浏览器。
...
```text
web-source-inspector-0.1.0.tgz
6462122E61447BE042769075BAC1D37737E2143D8F9D08165E56ECB4058CE10C
